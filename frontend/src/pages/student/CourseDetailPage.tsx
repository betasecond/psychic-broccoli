...
const CourseDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setCourse(courseRes)
        const chapterList = Array.isArray(chaptersRes) ? chaptersRes : (chaptersRes as any)?.chapters || []
        ...
        await Promise.all(
          chapterList.map(async (chapter: any) => {
            try {
              sectionMap[chapter.id] = await courseService.getSections(courseId, chapter.id)
            } catch {
        ...
        const myCourse = myCoursesRes.courses.find((item: any) => item.id === courseId)
        if (myCourse) {
  ...
  const handleAsk = async () => {
    ...
    setQuestion('')
    setQaList(prev => [...prev, { question: q, answer: '', sources: [], loading: true }])
    setAsking(true)
  ...
  const sectionTypeTag = (type: string) =>
    type === 'VIDEO' ? (
      <Tag icon={<PlayCircleOutlined />} color="blue" style={{ marginRight: 0 }}>
        视频
      </Tag>
    ) : (
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0', marginBottom: 12 }}>
        {qaList.length === 0 && (
          <Empty
            ...
            description={<Text type="secondary">输入课程相关问题，系统会基于知识库返回回答和来源。</Text>}
            style={{ marginTop: 60 }}
          />
        )}
                ...
                renderItem={item => (
                  ...
                  >
                    <List.Item.Meta
                      ...
                      description={<Text type="secondary" style={{ fontSize: 12 }}>{item.created_at?.slice(0, 10)}</Text>}
                    />
                  </List.Item>
                )}
              />
                ...
                items={[
                  {
                    ...
                    children: (
                      <div>
                        {chapters.length > 0 ? (
                          <Collapse accordion={false} expandIcon={({ isActive }) => <DownOutlined rotate={isActive ? 180 : 0} />}>
                                ...
                                <Panel
                                  ...
                                  header={
                                    <Space>
                                      ...
                                      {chapterSections.length > 0 && <Tag color="default">{chapterSections.length} 课时</Tag>}
                                    </Space>
                                  }
                                  extra={
                                    ...
                                    <List
                                      ...
                                      renderItem={section => (
                                        <List.Item
                                          key={section.id}
                                          style={{
                                            cursor: section.type === 'VIDEO' || section.type === 'TEXT' ? 'pointer' : 'default',
                                            background: activeSection?.id === section.id ? '#f0f5ff' : 'transparent',
                                            borderRadius: 4,
                                            padding: '8px 12px',
                                          }}
                                        ...
                                        >
                                          <List.Item.Meta
                                            avatar={sectionTypeTag(section.type)}
                                            title={<Text style={{ fontSize: 14 }}>{section.orderIndex}. {section.title}</Text>}
                                          />
                                        </List.Item>
                                      )}
                                    />
                                  )}
                                  {chapterSections.some(section => section.id === activeSection?.id) && activeSection && (
                                    <div style={{ marginTop: 12, padding: 16, background: '#fafafa', borderRadius: 8 }}>
                                      ...
                                      {activeSection.type === 'VIDEO' && activeSection.videoUrl ? (
                                        ...
                                        <Paragraph style={{ whiteSpace: 'pre-wrap', marginBottom: 0 }}>{activeSection.content}</Paragraph>
                                      ) : (
          ...
          <Card title="课程信息">
            <Space direction="vertical" size="middle" style={{ width: '100%' }}>
              <div>
                ...
                <div style={{ marginTop: 8 }}>
                  <Text type="secondary">{completedChapters.size} / {chapters.length} 章节已完成</Text>
                </div>
              </div>
              ...
              <div>
                ...
                <div>
                  {course.categoryName ? <Tag color="blue">{course.categoryName}</Tag> : <Text type="secondary">未分类</Text>}
                </div>
              </div>
