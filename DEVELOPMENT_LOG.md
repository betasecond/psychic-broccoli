...
### 社交热度算法优化与权重映射实现
**角色**: 前哨监军 (Monitor) / 魏延 (WeiYan)  
**目标**: 解决 `SOCIAL_TEST_CASES.md` 中的 CACHE-001 热度计算项。
...
**主要变更**:
1. **热度算法重构**:
   - 引入 **收藏 (1.5) > 回复 (1.0) > 点赞 (0.5) > 浏览 (0.2)** 的精细权重体系。
   - 采用自然对数时间衰减：`timeFactor = (ln(Hours+1.5)+1)^1.8`。这种改进后的算法使新帖在发布后的前 48 小时内能获得极高的曝光溢价，且分值随时间平滑衰减，避免旧帖永久霸榜。

2. **行为权重自动映射**:
   - 修改 `LikeReply` 和 `FavoriteReply` 接口：现在对**回复**的点赞或收藏，也会按一定比例自动累加到**主帖**对应的分值统计中。
   - 确保主帖热度实时感知子回复的活跃度。

3. **健壮性优化**:
   - 修复了异步热度计算 (`go refreshDiscussionHeat`) 调用时的参数传递顺序。
   - 在事务提交前完成主帖计数的原子更新，保证数据最终一致性。

**审查结论**:
- 算法逻辑符合“子午谷”突袭的高互动性要求。
- 代码符合郭嘉的“守土有责”原则，操作均在原子事务内完成。
